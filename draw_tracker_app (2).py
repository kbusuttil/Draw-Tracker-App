...
if pdf_files and inspections_file and sage_file:
    with st.spinner("Processing files..."):
        ...
        for file in pdf_files:
            ...
            # Extract Address - skip headers or filler lines
            address_line = next((line.strip() for line in lines if re.search(r'\d+\s+.*\s+(Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Place|Pl|Drive|Dr|Court|Ct)', line)), "")

            # Extract Approved Amount (look for line with "Approved Draw Release Amount")
            amount = 0
            for line in lines:
            ...
            # Extract Draw #
            draw_number = next((line for line in lines if re.search(r'Draw\s+#?\d+', line, re.IGNORECASE)), "Draw 1")

            # Fuzzy match to inspections for Loan ID
