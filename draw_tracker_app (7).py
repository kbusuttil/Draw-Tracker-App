...
if pdf_files and inspections_file and sage_file:
    with st.spinner("Processing files..."):
        ...
        def is_plausible_address(line):
            return bool(re.search(r'\d+\s+.*\b(Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Place|Pl|Drive|Dr|Court|Ct|NW|SW|NE|SE)\b', line))
        ...
        for file in pdf_files:
            ...
            # Try to find the best line that looks like an address
            address_line = ""
            for line in lines:
                if is_plausible_address(line) and len(line.split()) > 2:
                    address_line = line.strip()
                    break

            # Extract draw amount (primary method)
            amount = 0.0
            for line in lines:
            ...
            # Fallback: use the largest $ amount in the PDF
            if amount == 0.0:
                dollar_amounts = [float(match.replace(",", "")) for match in re.findall(r'\$([\d,]+\.\d{2})', text)]
                if dollar_amounts:
                    amount = max(dollar_amounts)

            # Extract draw number and format
            ...
            # Confidence flag
            confidence = "✅"
            if not address_line or not loan_id or amount == 0.0:
                confidence = "⚠️ Needs Review"
